...
#if defined(PROMINI)
  #if defined(NRF24_RX)
    uint8_t PWM_PIN[8] = {9,10,6,3,5,A2,12,11};   //use pin 6 instead of 11 - only valid for quad!
  #else
...
void writeMotors() { // [1000;2000] => [125;250]
  ...
  /********  Specific PWM Timers & Registers for the atmega328P (Promini)   ************/
    ...
    //motor[3] = (motor[3]-1000) << 1;    
      ...
      #if defined(NRF24_RX)
        #ifndef EXT_MOTOR_RANGE 
          atomicPWM_PIN6_highState = motor[1]>>3; // pin 6
        #else
...
void initOutput() {
  ...
  /********  Specific PWM Timers & Registers for the atmega328P (Promini)   ************/
    ...
    #if (NUMBER_MOTOR > 2)
      #if defined(NRF24_RX)     
        initializeSoftPWM(); // use pin 6
        //TCCR0A = _BV(WGM00) | _BV(WGM01) | _BV(COM0A1);  // Fast PWM, clear OC0A on compare match
        //TCCR0B = _BV(CS01);  // Prescaler 8 (490Hz)
        //OCR0A = 40;         
      #else
